...
function phase11_route(string $path,string $method,array $user): bool
{
    ...
    if($path==='/goals'){
        if($method==='POST'){
            ...
            $systemRaw=trim((string)($_POST['trading_system_id']??''));
            $systemId=$systemRaw===''?null:(filter_var($systemRaw,FILTER_VALIDATE_INT)?:0);
        ...
        render('goals',['title'=>'P&L Goals','goals'=>$goals,'accounts'=>$accounts,'systems'=>$systems]);return true;
    }
    return false;
}

function phase11_period_pnl(PDO $db,int $uid,string $start,string $end,?int $accountId=null,?int $systemId=null): float
{
    ...
    if($systemId!==null){$where.=' AND t.trading_system_id=?';$params[]=$systemId;}
