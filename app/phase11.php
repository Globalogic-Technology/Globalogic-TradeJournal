...
function phase11_route(string $path,string $method,array $user): bool
{
    ...
    if($path==='/goals'){
        if($method==='POST'){
            ...
            $q=$db->prepare('SELECT id FROM accounts WHERE id=? AND user_id=?');$q->execute([$accountId,$uid]);if(!$q->fetchColumn())throw new InvalidArgumentException('Select a valid account.');
            ...
            $s=$db->prepare('INSERT INTO pnl_goals(user_id,account_id,daily_goal,weekly_goal,monthly_goal,yearly_goal) VALUES(?,?,?,?,?,?) ON DUPLICATE KEY UPDATE daily_goal=VALUES(daily_goal),weekly_goal=VALUES(weekly_goal),monthly_goal=VALUES(monthly_goal),yearly_goal=VALUES(yearly_goal)');$s->execute([$uid,$accountId,...$vals]);flash('success','P&L goals saved.');redirect('/goals');
        }
...
function phase11_performance(PDO $db,int $uid): array
{
    ...
    $accounts=[];$s=$db->prepare('SELECT id,name,currency FROM accounts WHERE user_id=? ORDER BY name');$s->execute([$uid]);$accounts=$s->fetchAll();
    ...
    $s=$db->prepare('SELECT g.*,a.name account_name,a.currency FROM pnl_goals g JOIN accounts a ON a.id=g.account_id WHERE g.user_id=? ORDER BY a.name');$s->execute([$uid]);$goals=$s->fetchAll();
    return compact('accounts','accountPnl','calendar','year','goals','today','weekStart','monthStart','yearStart');
}
