...
function phase3_trade_form_data(array $input, array $user): array
{
    ...
    $accountStmt = $db->prepare('SELECT id,default_system_id FROM accounts WHERE id=? AND user_id=?');
    ...
    if (!$assetId) {
        ...
        if ($symbol !== '') {
            $assetStmt = $db->prepare('SELECT id FROM assets WHERE user_id=? AND UPPER(symbol)=? LIMIT 1');
            $assetStmt->execute([$userId, $symbol]);
            $matchedAsset = $assetStmt->fetchColumn();
            if ($matchedAsset) $assetId = (int)$matchedAsset;
        }
    }
...
function phase3_trade_route(string $path, string $method, array $user): bool
{
    ...
    if ($method === 'POST') {
        ...
        if ($id) {
            $stmt = $db->prepare('UPDATE trades SET account_id=?,ticket=?,symbol=?,side=?,status=?,opened_at=?,closed_at=?,quantity=?,entry_price=?,stop_loss=?,take_profit=?,exit_price=?,fees=?,notes=?,trading_system_id=?,strategy_id=?,asset_id=?,trading_session_id=? WHERE id=? AND user_id=?');
            ...
            $stmt = $db->prepare('INSERT INTO trades(account_id,ticket,symbol,side,status,opened_at,closed_at,quantity,entry_price,stop_loss,take_profit,exit_price,fees,notes,trading_system_id,strategy_id,asset_id,trading_session_id,user_id) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)');
    ...
    if($editId){$s=$db->prepare('SELECT * FROM trades WHERE id=? AND user_id=?');$s->execute([$editId,$userId]);$editTrade=$s->fetch()?:null;}
    $newTradeDefaults=[];
    if(!$editTrade && $accounts){$newTradeDefaults=['account_id'=>(int)$accounts[0]['id'],'trading_system_id'=>(int)($accounts[0]['default_system_id']??0)];}

    $where=['t.user_id=?']; $params=[$userId];
    $symbol=trim((string)($_GET['symbol']??'')); $side=$_GET['side']??''; $status=$_GET['status']??'';
    $systemFilter=filter_var($_GET['trading_system_id']??null,FILTER_VALIDATE_INT); $assetFilter=filter_var($_GET['asset_id']??null,FILTER_VALIDATE_INT);
    ...
    $s=$db->prepare('SELECT t.*,a.name account_name,a.currency,a.initial_balance,ts.name system_name,st.name strategy_name,ar.symbol asset_symbol,sess.name session_name FROM trades t JOIN accounts a ON a.id=t.account_id AND a.user_id=t.user_id LEFT JOIN trading_systems ts ON ts.id=t.trading_system_id AND ts.user_id=t.user_id LEFT JOIN strategies st ON st.id=t.strategy_id AND st.user_id=t.user_id LEFT JOIN assets ar ON ar.id=t.asset_id AND ar.user_id=t.user_id LEFT JOIN trading_sessions sess ON sess.id=t.trading_session_id AND sess.user_id=t.user_id WHERE '.implode(' AND ',$where).' ORDER BY t.opened_at DESC,t.id DESC LIMIT 25 OFFSET '.$offset);
    $s->execute($params);$trades=$s->fetchAll();

    $riskService=new \App\Services\TradeRiskService();
    foreach($trades as &$t){$t['pnl']=trade_pnl($t);$balanceBefore=(float)$t['initial_balance'];$prior=$db->prepare('SELECT * FROM trades WHERE user_id=? AND account_id=? AND status="closed" AND closed_at IS NOT NULL AND (closed_at < ? OR (closed_at = ? AND id < ?)) ORDER BY closed_at,id');$prior->execute([$userId,$t['account_id'],$t['closed_at']??$t['opened_at'],$t['closed_at']??$t['opened_at'],$t['id']]);foreach($prior->fetchAll() as $p){$balanceBefore+=(float)(trade_pnl($p)??0);}$t['risk']=$riskService->calculate($db,$userId,$t,$balanceBefore);}unset($t);

    $editRisk=null;
    if($editTrade){$balanceBefore=(float)($editTrade['initial_balance']??0);$a=$db->prepare('SELECT initial_balance FROM accounts WHERE id=? AND user_id=?');$a->execute([(int)$editTrade['account_id'],$userId]);$balanceBefore=(float)($a->fetchColumn()?:$balanceBefore);$editRisk=$riskService->calculate($db,$userId,$editTrade,$balanceBefore);}
    render('trades',['title'=>'Trades','accounts'=>$accounts,'systems'=>$systems,'strategies'=>$strategies,'assets'=>$assets,'sessions'=>$sessions,'editTrade'=>$editTrade,'newTradeDefaults'=>$newTradeDefaults,'editRisk'=>$editRisk,'trades'=>$trades,'filters'=>['symbol'=>$symbol,'side'=>$side,'status'=>$status,'trading_system_id'=>$systemFilter,'asset_id'=>$assetFilter],'page'=>$page,'pages'=>$pages,'total'=>$total]);
    return true;
}
