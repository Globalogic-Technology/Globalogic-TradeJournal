...
{
    $db=db();$uid=(int)$user['id'];$s=$db->prepare('SELECT id,name,currency FROM accounts WHERE user_id=? ORDER BY name');$s->execute([$uid]);$accounts=$s->fetchAll();$result=null;$preview=null;
    if($method==='POST'){
