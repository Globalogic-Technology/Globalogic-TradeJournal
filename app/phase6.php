...
function phase6_import(string $method, array $user): void
{
    ...
    if($method==='POST'){
        verify_csrf();$action=(string)($_POST['action']??'import');$accountId=(int)($_POST['account_id']??0);$check=$db->prepare('SELECT id,name FROM accounts WHERE id=? AND user_id=?');$check->execute([$accountId,$uid]);$account=$check->fetch();if(!$account)throw new InvalidArgumentException('Invalid account.');
        ...
        $service=new CsvImportService();$delimiter=(string)$template['delimiter_char'];$hasHeader=(bool)$template['has_header'];$timezone=(string)$template['date_timezone'];
        $accountIsInteractiveBrokers=str_contains(strtolower((string)$account['name']),'interactive brokers');
        // Smart detection belongs to the Import page. The template's manual mappings are
        // always passed through so the smart importer can use them before its IB fallbacks.
        $smartEnabled=$accountIsInteractiveBrokers && (($template['import_mode']??'standard')==='smart_csv');
        $mode=$smartEnabled?'interactive_brokers_smart':'standard';
        if($action==='preview'){$preview=$service->previewMapped($file['tmp_name'],$mapping,$delimiter,$hasHeader,$timezone,$mode);AuditService::log($db,$uid,'csv_preview','account',$accountId,'CSV preview requested using template '.$template['name'].($smartEnabled?' with Interactive Brokers smart fallback':''));}
