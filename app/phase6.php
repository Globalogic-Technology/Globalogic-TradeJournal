...
function phase6_import(string $method, array $user): void
{
    ...
    if($method==='POST'){
        ...
        // Smart broker logic is deliberately decided here, on the Import page. The Accounts page remains the place for manual mappings.
        $mode=$accountIsInteractiveBrokers?'interactive_brokers_smart':'standard';
        if($action==='preview'){$preview=$service->previewMapped($file['tmp_name'],$mapping,$delimiter,$hasHeader,$timezone,$mode);AuditService::log($db,$uid,'csv_preview','account',$accountId,'CSV preview requested using template '.$template['name'].($accountIsInteractiveBrokers?' with Interactive Brokers smart fallback':''));}
