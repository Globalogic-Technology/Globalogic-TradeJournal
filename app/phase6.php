...
function phase6_import(string $method, array $user): void
{
    ...
    $templates=[];$q=$db->prepare('SELECT id,account_id,name,delimiter_char,has_header,date_timezone,mapping_json,is_default FROM account_csv_templates WHERE user_id=? ORDER BY account_id,is_default DESC,name');$q->execute([$uid]);
