...
final class AccountCsvTemplateService
{
    ...
    public static function save(PDO $db,int $userId,int $accountId,?int $id,string $name,string $delimiter,bool $hasHeader,string $timezone,array $mapping,bool $default,string $importMode='standard'): int
    {
        ...
        $owned=$db->prepare('SELECT id FROM accounts WHERE id=? AND user_id=?');$owned->execute([$accountId,$userId]);
        if(!$owned->fetchColumn())throw new RuntimeException('Invalid account.');

        // Smart CSV Import still stores the user's manual mappings. The Import page uses
        // those mappings first and only falls back to broker-aware smart detection for gaps.
        $json=json_encode(self::normalizeMapping(json_encode($mapping,JSON_THROW_ON_ERROR),$importMode==='standard'),JSON_THROW_ON_ERROR);
        $db->beginTransaction();
        try{
            ...
            if($id){
                $q=$db->prepare('SELECT id FROM account_csv_templates WHERE id=? AND account_id=? AND user_id=?');$q->execute([$id,$accountId,$userId]);
    ...
    {
        $s=$db->prepare('DELETE FROM account_csv_templates WHERE id=? AND user_id=?');$s->execute([$id,$userId]);
