...
final class NotificationService
{
    public static function generate(PDO $db, int $userId): int
    {
        ...
        $exists=$db->prepare('SELECT id FROM user_notifications WHERE user_id=? AND type="journal_due" AND entity_id=? LIMIT 1');
        $add=$db->prepare('INSERT INTO user_notifications(user_id,type,title,message,entity_type,entity_id) VALUES(?,?,?,?,?,?)');
