...
<div class="card form-card">
  ...
  <?php else: ?>
    <div class="table-wrap">
      ...
      <?php foreach($accounts as $a): $count=count(array_filter($csvTemplates,fn($t)=>(int)$t['account_id']===(int)$a['id'])); ?>
        <tr>
          <td><strong><?=e($a['name'])?></strong></td><td><span class="badge"><?=e($a['currency'])?></span></td><td><?=number_format((float)$a['initial_balance'],2)?></td><td><?=number_format($count)?></td>
          <td class="trade-actions"><a href="/accounts?edit_account=<?=e($a['id'])?>">Edit</a><form method="post" class="inline-form"><input type="hidden" name="_csrf" value="<?=e(csrf_token())?>"><input type="hidden" name="action" value="delete"><input type="hidden" name="id" value="<?=e($a['id'])?>"><button class="danger">Delete</button></form></td>
        </tr>
      <?php endforeach; ?>
      </tbody></table>
    </div>
...
<div class="card form-card">
  <div class="section-heading"><div><h2>Trade CSV template list</h2><div class="sub">All configured templates across your accounts.</div></div></div>
  <?php if(!$csvTemplates): ?>
    <p class="muted">No Trade CSV templates configured yet.</p>
  <?php else: ?>
    <div class="table-wrap">
      <table><thead><tr><th>Account</th><th>Template</th><th>Default</th><th>Delimiter</th><th>Header</th><th>Time zone</th><th>Mapped fields</th><th>Actions</th></tr></thead><tbody>
      <?php foreach($csvTemplates as $t): $accountName='';foreach($accounts as $a)if((int)$a['id']===(int)$t['account_id']){$accountName=$a['name'];break;} $mapped=0;foreach(($t['mapping']??[]) as $column)if(trim((string)$column)!=='')$mapped++; ?>
        <tr>
          <td><strong><?=e($accountName ?: 'Unknown account')?></strong></td>
          <td><?=e($t['name'])?></td>
          <td><?php if($t['is_default']): ?><span class="badge">Default</span><?php else: ?>No<?php endif; ?></td>
          ...
          <td class="trade-actions"><a href="/accounts?edit_csv_template=<?=e($t['id'])?>">Edit</a><form method="post" class="inline-form"><input type="hidden" name="_csrf" value="<?=e(csrf_token())?>"><input type="hidden" name="action" value="csv_template_delete"><input type="hidden" name="template_id" value="<?=e($t['id'])?>"><button class="danger">Delete</button></form></td>
        </tr>
      <?php endforeach; ?>
      </tbody></table>
    </div>
