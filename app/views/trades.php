...
<div class="table-wrap"><table><tr><th>Ticket</th><th>Symbol</th><th>Side</th><th>Status</th><th>System</th><th>Strategy</th><th>Asset</th><th>Session</th><th>Opened</th><th>Qty</th><th>Entry</th><th>Exit</th><th>P&amp;L</th><th></th></tr>
<?php foreach($trades as $t):?><tr>
<td><?=e($t['ticket']??'')?></td><td><?=e($t['symbol'])?></td><td><?=e($t['side'])?></td><td><?=e($t['status'])?></td><td><?=e($t['system_name']??'—')?></td><td><?=e($t['strategy_name']??'—')?></td><td><?=e($t['asset_symbol']??'—')?></td><td><?=e($t['session_name']??'—')?></td><td><?=e($t['opened_at'])?></td><td><?=e($t['quantity'])?></td><td><?=e($t['entry_price'])?></td><td><?=e($t['exit_price']??'')?></td><td class="<?=($t['pnl']===null?'':($t['pnl']>=0?'positive':'negative'))?>"><?=$t['pnl']===null?'—':number_format($t['pnl'],2)?></td>
...
<?php if(!$trades):?><tr><td colspan="14" class="muted">No trades found.</td></tr><?php endif;?></table></div>
