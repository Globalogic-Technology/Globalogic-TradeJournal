...
<p><label>Asset</label><select name="asset_id"><option value="">All</option><?php foreach($assets as $a):?><option value="<?=e($a['id'])?>" <?=((int)($filters['asset_id']??0)===(int)$a['id'])?'selected':''?>><?=e($a['symbol'])?></option><?php endforeach;?></select></p>
<p style="align-self:end"><button>Filter</button></p>
</form>
<div class="table-wrap"><table class="trade-history"><tr><th>Ticket</th><th>Symbol</th><th>Side</th><th>Status</th><th>System</th><th>Strategy</th><th>Asset</th><th>Session</th><th>Opened</th><th>Quantity</th><th>Entry</th><th>Exit</th><th>P&amp;L</th><th>Ideal Risk</th><th>Actual Risk</th><th>Expected R</th><th>R Multiple</th><th>Risk Dev.</th><th>Balance After</th><th></th></tr>
<?php foreach($trades as $t):$r=$t['risk'];?><tr>
<td><?=e($t['ticket']??'')?></td><td><?=e($t['symbol'])?></td><td><?=e($t['side'])?></td><td><?=e($t['status'])?></td><td><?=e($t['system_name']??'—')?></td><td><?=e($t['strategy_name']??'—')?></td><td><?=e($t['asset_symbol']??'—')?></td><td><?=e($t['session_name']??'—')?></td><td><?=e($t['opened_at'])?></td><td class="quantity"><?=number_format((float)$t['quantity'],4,'.','')?></td><td><?=number_format((float)$t['entry_price'],5,'.','')?></td><td><?=$t['exit_price']===null?'—':number_format((float)$t['exit_price'],5,'.','')?></td><td class="<?=($t['pnl']===null?'':($t['pnl']>=0?'positive':'negative'))?>"><?=$t['pnl']===null?'—':number_format($t['pnl'],2)?></td>
...
.trade-history th:nth-child(10),.trade-history td:nth-child(10){min-width:100px;text-align:right}
