...
try {
    if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
        ...
        if ($action === 'csv_template_delete') {
            ...
            flash('success','CSV template deleted.'); redirect('/accounts');
        }

        $id=filter_var($_POST['id']??null,FILTER_VALIDATE_INT)?:0;
        $name=trim((string)($_POST['name']??''));$currency=strtoupper(trim((string)($_POST['currency']??'USD')));
        ...
        if($action==='create'){$db->prepare('INSERT INTO accounts(user_id,name,currency,initial_balance) VALUES(?,?,?,?)')->execute([$uid,$name,$currency,$balance]);flash('success','Account created.');}
        elseif($action==='update'&&$id){$db->prepare('UPDATE accounts SET name=?,currency=?,initial_balance=? WHERE id=? AND user_id=?')->execute([$name,$currency,$balance,$id,$uid]);flash('success','Account updated.');}
        elseif($action==='delete'&&$id){$db->prepare('DELETE FROM accounts WHERE id=? AND user_id=?')->execute([$id,$uid]);flash('success','Account deleted.');}
    ...
    if($editAccountId)foreach($accounts as $account)if((int)$account['id']===$editAccountId){$editAccount=$account;break;}
